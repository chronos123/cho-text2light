...
def load_model(config, ckpt, device, eval_mode):
    ...
    model = load_model_from_config(config.model, state_dict, device=device, eval_mode=eval_mode)["model"]
    return model
...
def main_worker(rank, ngpus_per_node, opt, unknown, prompts):
    # set the device
    torch.cuda.set_device(rank)
    device = torch.device('cuda', rank)
    gpu = False  # we will specify the device directly
    eval_mode = True
    show_config = False
    ...
    global_sampler = load_model(config, ckpt, device, eval_mode)
    ...
    local_sampler = load_model(config, ckpt, device, eval_mode)

    # Load other resources (e.g., clip_emb, knn_index)
    ...
    # Prepare prompts for this worker
    prompt_chunks = chunkify(prompts, ngpus_per_node)
    my_prompts = prompt_chunks[rank]
    ...
    # Process prompts in batches of size opt.bs_per_gpu
    bs = opt.bs_per_gpu
    for i in tqdm(range(0, len(my_prompts), bs)):
        end_i = min(len(my_prompts), i + bs)
        prompt_batch = my_prompts[i:end_i]
        text2light(input_models, prompt_batch, outdir, input_params, rank*10000 + i)
...
if __name__ == "__main__":
    ...
    ngpus_per_node = opt.ngpu
    gpu = True if opt.ngpu > 0 else False
    ...
    if ngpus_per_node > 1:
        mp.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, opt, unknown, prompts))
    else:
        main_worker(0, ngpus_per_node, opt, unknown, prompts)


# python script.py --ngpu 4 --bs_per_gpu 2 --resume_global logs/2024-10-19T15-20-22_global_sampler_clip_ours/checkpoints/epoch=000245.ckpt --resume_local logs/2024-10-20T14-02-08_local_sampler_spe_ours/checkpoints/epoch=000197.ckpt --clip clip_emb_ours.npy --text /data1/sxc/VQ_generation/scripts/test_prompts.txt --outdir logs/samples/raw
